...
//        vf_main.setBackgroundDrawable(res.getDrawable(R.drawable.background));        
        main = vf_main.findViewById(R.id.ll_main);        
        setContentView(vf_main);
        setup = new ViewSetup(context, settings, main, vf_main, inputMethodManager);    
...
//    	menu.findItem(R.id.oi_settings).setVisible(false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {    	
    	switch(item.getItemId()) {
    		...
    		case R.id.oi_show_saved_alt:
    			if (ThrillingTales.SAVED_SHOWN == 0  || ThrillingTales.SAVED_SHOWN == -1)
	...
	protected boolean handleQuit(Boolean forceQuit){
		...
		if (description_view != null && description_view.getVisibility() == View.VISIBLE && forceQuit != true){
			vf_main.showPrevious();
			vf_main.removeView(description_view);
			description_view = null;
			return false;
		} else if (about_view != null && about_view.getVisibility() == View.VISIBLE && forceQuit != true){
			vf_main.showPrevious();
